...
public class Cipher   
{
    ...
    public String encrypt(String inputString) {
        ...
        {
            outputString = outputString + replaceChar(inputString.charAt(i), true); //CHANGED - ertugrul
        }
    ...
    public String decrypt(String inputString) {
        ...
        //replaceChar('a',true); //DELETED - ertugrul

        for (int i = 0; i < inputString.length(); i++)   
        {
            outputString = outputString + replaceChar(inputString.charAt(i),false); //ADDED - ertugrul
        }
        ...
        if(isEncrypt) {
            for (int i = 0; i < ORIGINAL_ALPHABET.length(); i++)   
            {
                if(ORIGINAL_ALPHABET.charAt(i) == inputChar) {
                    return CIPHER_ALPHABET.charAt(i); //CHANGED - ertugrul
                }
            }
        }
        else {
