...
public class Cipher   
{
    ...
    public String encrypt(String inputString) {
        ...
        {   if(inputString.charAt(i) != ' '){
                for(int m = 0; m < ORIGINAL_ALPHABET.length(); m++){
                
                    if( inputString.charAt(i) == ORIGINAL_ALPHABET.charAt(m)){
                    outputString += CIPHER_ALPHABET.charAt(m);
                    }
    ...
    public String decrypt(String inputString) {
        ...
        for(int i = 0; i < inputString.length(); i++){
            if(inputString.charAt(i) != ' '){
            char charToReplace = inputString.charAt(i);
            char replacedChar = replaceChar(charToReplace,false);
